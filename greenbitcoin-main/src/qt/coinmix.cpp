...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::GBTC)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("GBTC")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizegreenbitcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizegreenbitcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizegreenbitcoinAmount = strAnonymizegreenbitcoinAmount.remove(strAnonymizegreenbitcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegreenbitcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizegreenbitcoinAmount * COIN) nMaxToAnonymize = nAnonymizegreenbitcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizegreenbitcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizegreenbitcoinAmount));
        strAnonymizegreenbitcoinAmount = strAnonymizegreenbitcoinAmount.remove(strAnonymizegreenbitcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegreenbitcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void Coinmix::toggleDarksend()
{
    ...
    if (!fEnableDarksend) {
        ...
        if (nAnonymizegreenbitcoinAmount == 0) {
